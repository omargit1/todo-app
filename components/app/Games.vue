    ...
    <v-row no-gutters align="center">
      <template v-for="n in cases">
        <v-col :key="n.num" class="col-3" align="center">
          <v-card @click.stop="isClicked(n)" :disabled="caseIsClicked(n)">
            <v-card-title
              class="case"
              :style="{'backgroundColor' : n.num == numClickedCurrent || n.isActive ? n.color : 'blue' }"
            >{{ n.isActive ? ( n.color == 'green' ? '+'+ n.num : '-' + n.num) : '' }}</v-card-title>
          </v-card>
        </v-col>
      </template>
    </v-row>
    <v-row align="center">
      <v-col align="center" class="col-4">
        <v-card style="height:60px">
          <v-card-title style="height:50px" class="justify-center">Score</v-card-title>
        </v-card>
      </v-col>
      <v-col align="center">
        <v-card style="height:60px">
          <v-card-title style="height:50px;font-size:3rem" class="justify-center">{{ score }}</v-card-title>
        </v-card>
      </v-col>
    </v-row>
  </v-container>
...
export default {
    ...
    methods : {
        ...
        caseIsClicked(n) {
            console.log(n.num)
           return this.arrayClicked.includes(n.num)
